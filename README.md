...
| --- | --- | --- |
| ** ***In Progress*** ** [Recommendation API](https://github.com/04mscott/Recommendation-API) | Python, FastAPI, Sklearn, Vector embeddings, MySQL, AWS, Relational databases | Implemented an API that pulls a users song data from Spotify and stores it in a database, as well as vector embeddings, which are used to make recommendations using cosine similarity. The MySQL database is then used to cross reference top songs, saved songs, etc. for each user |
